...
export default function SchedulePage() {
  ...
  const createGroup = async (e) => {
    ...
    if (groupForm.days_of_week.length === 0) {
      setGroupError('Select at least one active day')
      return
    }
    ...
    if ((groupForm.start_time && normalizedStartTime === null) || (groupForm.end_time && normalizedEndTime === null)) {
      setGroupError('Invalid time format. Use 1, 13, or 1300')
      return
    }

    if ((normalizedStartTime && !normalizedEndTime) || (!normalizedStartTime && normalizedEndTime)) {
      setGroupError('Enter both start and end time')
      return
    }
    ...
    if (normalizedStartTime && normalizedEndTime) {
      ...
      if (startMinutes < minAllowed || startMinutes > maxAllowed || endMinutes < minAllowed || endMinutes > maxAllowed) {
        setGroupError('Activity hours must be between 07:00 and 22:00')
        return
      }

      if (endMinutes <= startMinutes) {
        setGroupError('End time must be after start time')
        return
      }
    }
  ...
  if (!coach) return <div className="p-5 text-center text-muted-foreground">Loading...</div>

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-extrabold mb-1">Schedule</h1>
        <p className="text-muted-foreground text-sm">Welcome, {coach?.name}</p>
      </div>
      ...
      <div className="flex gap-2 justify-center mb-4">
        <Button variant={filterMode === 'all' ? 'default' : 'outline'} onClick={() => setFilterMode('all')}>All coaches</Button>
        <Button variant={filterMode === 'my' ? 'default' : 'outline'} onClick={() => setFilterMode('my')}>My only</Button>
      </div>

      {loading ? <div className="text-center p-5 text-muted-foreground">Loading...</div> : null}
              ...
              <Card key={day.date}>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" onClick={() => openAddGroupModal(day.dateObj)}>+ Group</Button>
                  </div>
                  <div className="mt-3">
                    {daySessions.length > 0 ? daySessions.map((session) => (
                      <div key={session.id} className="bg-secondary border border-border rounded-lg p-3 mt-2 cursor-pointer" onClick={() => { setSelectedSession(session); setDetailModalOpen(true) }}>
                        <div className="text-sm font-semibold">{session.groups?.name || 'Group'}</div>
                        <div className="text-xs text-muted-foreground">{session.start_time || 'No time'}</div>
                      </div>
      ...
      {!loading && viewMode === 'day' ? (
        <div className="space-y-3">
          <div className="flex justify-end"><Button size="sm" onClick={() => openAddGroupModal(currentDate)}>Add group on this day</Button></div>
          {currentDaySessions.length === 0 ? <div className="text-muted-foreground text-center p-5">No events on this day</div> : currentDaySessions.map((session) => (
            <Card key={session.id} className="cursor-pointer" onClick={() => { setSelectedSession(session); setDetailModalOpen(true) }}>
              <CardContent className="p-4">
                <div className="text-sm font-bold">{session.groups?.name || 'Group'}</div>
                <div className="text-xs text-muted-foreground">{session.date} • {session.start_time || 'No time'}</div>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <Dialog open={groupModalOpen} onOpenChange={setGroupModalOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <form className="space-y-4" onSubmit={createGroup}>
            <div><Label>Group Name *</Label><Input value={groupForm.name} onChange={(e) => setGroupForm((s) => ({ ...s, name: e.target.value }))} required /></div>
            <div><Label>Color</Label><div className="mt-2 flex flex-wrap gap-2">{GROUP_COLORS.map((color) => (
            ...
            <div><Label>Active Days</Label><div className="mt-2 grid grid-cols-7 gap-1.5">{DAY_SHORT.map((label, dayIndex) => {
            ...
            <div><Label>Activity Hours</Label><div className="mt-2 grid grid-cols-2 gap-2"><Input type="text" inputMode="numeric" value={groupForm.start_time} onChange={(e) => setGroupForm((s) => ({ ...s, start_time: e.target.value }))} /><Input type="text" inputMode="numeric" value={groupForm.end_time} onChange={(e) => setGroupForm((s) => ({ ...s, end_time: e.target.value }))} /></div></div>
            <div><Label>Start Date</Label><Input type="date" value={groupForm.start_date} onChange={(e) => setGroupForm((s) => ({ ...s, start_date: e.target.value }))} /></div>
            {groupError ? <div className="text-xs text-red-400">{groupError}</div> : null}
            <Button type="submit" className="w-full" disabled={groupSaving}>{groupSaving ? 'Saving...' : 'Create Group'}</Button>
          </form>
        </DialogContent>
      </Dialog>
